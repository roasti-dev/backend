...
func uploadImage(t *testing.T, c *client.ClientWithResponses, data []byte) string {
	...
	resp, err := c.PostApiV1UploadsImagesWithBodyWithResponse(t.Context(), nil, contentType, bytes.NewReader(body))
	require.NoError(t, err)
...
func TestGetImage(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		resp, err := c.GetApiV1UploadsImagesImageIdWithResponse(t.Context(), imageID, nil)
		require.NoError(t, err)
	...
	t.Run("not found", func(t *testing.T) {
		c := newTestClient(t, srv, "user-1")
		resp, err := c.GetApiV1UploadsImagesImageIdWithResponse(t.Context(), "non-existent-id", nil)
		require.NoError(t, err)
